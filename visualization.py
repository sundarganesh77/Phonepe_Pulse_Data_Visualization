...
connection = pymysql.connect(host='localhost',user='root',password='365pass',database='phonepe_pulse')
mycursor = connection.cursor()

#=================================================Dashboard===================================================================

#setting up page configuration
icon =Image.open('D:/Data Science/PhonePe/phonepe_pulse/phonepe.png')

st.set_page_config(page_title="Phonepe_Pulse",
                   page_icon=icon,
                   layout='wide',
                   initial_sidebar_state='expanded',
                   menu_items={'About':"""#Data from 2018 to 2022 has been cloned from Phonepe Pulse Github repository and sent to Mysql.
                                            Created a visualization dashboard using streamlit"""})
...
def main_page():
    ...
    with col1:
        image = Image.open('D:/Data Science/PhonePe/phonepe_pulse/ICN.png')
        st.image(image, width=105, use_column_width=False)
        # Adjust vertical alignment for centered image
        st.markdown('<p style="display: flex; justify-content:left;"</p>', unsafe_allow_html=True)

    with col2:
        st.title(':violet Phonepe Pulse Visualization Dashboard')
    st.write("Phonepe Data from 2018 to 2022 has been cloned from Phonepe Pulse Github repository and sent to Mysql.Created a visualization dashboard using streamlit")
    st.image(Image.open('D:/Data Science/PhonePe/phonepe_pulse/Phonepe_1.jpg'), caption='Architectural Diagram of the Project')

# Visualization page content
def visualization_page():
    st.title("Visualization Page")
    st.write("Welcome to the streamlit visualization page!")
    # Selection option
    option = st.radio('**Select your option**',('ALL INDIA', 'STATE','TOP 10 CATEGORIES'),horizontal=True)
    if option == 'ALL INDIA':
        all_india_transaction_tab()
        
    elif option == 'STATE':
        state_wise_transaction_tab()
        
    else:
        top_transaction_tab()
        

def all_india_transaction_tab():
    ...
    with tab1:
        ...
        mycursor.execute(f"SELECT State, Transaction_amount FROM aggregrated_transaction_details WHERE Year = '{in_tr_yr}' AND Quarter = '{in_tr_qtr}' AND Transaction_type = '{in_tr_tr_typ}';")
        ...
        mycursor.execute(f"SELECT State, Transaction_count, Transaction_amount FROM aggregrated_transaction_details WHERE Year = '{in_tr_yr}' AND Quarter = '{in_tr_qtr}' AND Transaction_type = '{in_tr_tr_typ}';")
        ...
        mycursor.execute(f"SELECT SUM(Transaction_amount), AVG(Transaction_amount) FROM aggregrated_transaction_details WHERE Year = '{in_tr_yr}' AND Quarter = '{in_tr_qtr}' AND Transaction_type = '{in_tr_tr_typ}';")
        ...
        mycursor.execute(f"SELECT SUM(Transaction_count), AVG(Transaction_count) FROM aggregrated_transaction_details WHERE Year = '{in_tr_yr}' AND Quarter = '{in_tr_qtr}' AND Transaction_type = '{in_tr_tr_typ}';")
    ...
    with tab2:
        ...
        mycursor.execute(f"SELECT State, SUM(User_Count) FROM aggregrated_user_details WHERE Year = '{in_us_yr}' AND Quarter = '{in_us_qtr}' GROUP BY State;")
        ...
        mycursor.execute(f"SELECT SUM(User_Count), AVG(User_Count) FROM aggregrated_user_details WHERE Year = '{in_us_yr}' AND Quarter = '{in_us_qtr}';")
...
def state_wise_transaction_tab():
    tab3, tab4 = st.tabs(['Transaction', 'User'])

    with tab3:
        col1, col2, col3 = st.columns(3)
        with col1:
            st_tr_st = st.selectbox('**Select State**',('andaman-&-nicobar-islands', 'andhra-pradesh', 'arunachal-pradesh','assam', 'bihar', 'chandigarh', 'chhattisgarh','dadra-&-nagar-haveli-&-daman-&-diu', 'delhi', 'goa', 'gujarat', 'haryana', 'himachal-pradesh', 'jammu-&-kashmir', 'jharkhand', 'karnataka', 'kerala', 'ladakh', 'lakshadweep', 'madhya-pradesh','maharashtra', 'manipur', 'meghalaya', 'mizoram', 'nagaland','odisha', 'puducherry', 'punjab', 'rajasthan', 'sikkim', 'tamil-nadu', 'telangana', 'tripura', 'uttar-pradesh', 'uttarakhand', 'west-bengal'),key='st_tr_st')
        with col2:
        ...
        # Transaction Analysis bar chart query
        mycursor.execute(f"SELECT Transaction_type, Transaction_amount FROM aggregrated_transaction_details WHERE State = '{st_tr_st}' AND Year = '{st_tr_yr}' AND Quarter = '{st_tr_qtr}';")
        ...
        # Transaction Analysis table query
        mycursor.execute(f"SELECT Transaction_type, Transaction_count, Transaction_amount FROM aggregrated_transaction_details WHERE State = '{st_tr_st}' AND Year = '{st_tr_yr}' AND Quarter = '{st_tr_qtr}';")
        ...
        # Total Transaction Amount table query
        mycursor.execute(f"SELECT SUM(Transaction_amount), AVG(Transaction_amount) FROM aggregrated_transaction_details WHERE State = '{st_tr_st}' AND Year = '{st_tr_yr}' AND Quarter = '{st_tr_qtr}';")
        ...
        # Total Transaction Count table query
        mycursor.execute(f"SELECT SUM(Transaction_count), AVG(Transaction_count) FROM aggregrated_transaction_details WHERE State = '{st_tr_st}' AND Year ='{st_tr_yr}' AND Quarter = '{st_tr_qtr}';")
    ...
    with tab4:
        col1, col2, col3 = st.columns(3)
        with col1:
            st_us_st = st.selectbox('**Select State**',('andaman-&-nicobar-islands', 'andhra-pradesh', 'arunachal-pradesh','assam', 'bihar', 'chandigarh', 'chhattisgarh','dadra-&-nagar-haveli-&-daman-&-diu', 'delhi', 'goa', 'gujarat', 'haryana', 'himachal-pradesh', 'jammu-&-kashmir', 'jharkhand', 'karnataka', 'kerala', 'ladakh', 'lakshadweep', 'madhya-pradesh','maharashtra', 'manipur', 'meghalaya', 'mizoram', 'nagaland','odisha', 'puducherry', 'punjab', 'rajasthan', 'sikkim', 'tamil-nadu', 'telangana', 'tripura', 'uttar-pradesh', 'uttarakhand', 'west-bengal'),key='st_us_st')
        with col2:
        ...
        # User Analysis Bar chart query
        mycursor.execute(f"SELECT Quarter, SUM(User_Count) FROM aggregrated_user_details WHERE State = '{st_us_st}' AND Year = '{st_us_yr}' GROUP BY Quarter;")
        ...
        # Total User Count table query
        mycursor.execute(f"SELECT SUM(User_Count), AVG(User_Count) FROM aggregrated_user_details WHERE State = '{st_us_st}' AND Year = '{st_us_yr}';")
...
def top_transaction_tab():
    tab5, tab6 = st.tabs(['Transaction', 'User'])

    with tab5:
        col1, col2 = st.columns(2)
        with col1:
            top_tr_cat = st.selectbox('**Select Category**', ('Recharge & bill payments', 'Peer-to-peer payments', 'Merchant payments', 'Financial Services', 'Others'), key='top_tr_cat')
        with col2:
        ...
        mycursor.execute(f"SELECT State, SUM(Transaction_amount) As Transaction_amount FROM top_transaction_detials WHERE Year = '{top_tr_yr}' GROUP BY State ORDER BY Transaction_amount DESC LIMIT 10;")
        ...
        mycursor.execute(f"SELECT State, SUM(Transaction_amount) as Transaction_amount, SUM(Transaction_count) as Transaction_count FROM top_transaction_detials WHERE Year = '{top_tr_yr}' GROUP BY State ORDER BY Transaction_amount DESC LIMIT 10;")
    ...
    with tab6:
        col1, col2 = st.columns(2)
        with col1:
            top_us_cat = st.selectbox('**Select Category**', ('Recharge & bill payments', 'Peer-to-peer payments', 'Merchant payments', 'Financial Services', 'Others'), key='top_us_cat')
        with col2:
        ...
        mycursor.execute(f"SELECT State, SUM(Registered_User) AS Top_user FROM top_user_details WHERE Year='{top_us_yr}' GROUP BY State ORDER BY Top_user DESC LIMIT 10;")
...
def main():
    ...
    page = st.sidebar.selectbox("Go to", ["Main Page", "Visualization Page"])
